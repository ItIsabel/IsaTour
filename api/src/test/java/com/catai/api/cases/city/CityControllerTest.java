...
public class CityControllerTest {
    ...
    void todasCiudades_DeberiaRetornarListaOrdenada_CuandoExistenCiudades() throws Exception {
        List<City> cities = Arrays.asList(city2, city1); // Unsorted
        ...
        mockMvc.perform(get("/ciudades"))
                ...
                .andExpect(jsonPath("$[0].nombre").value("Barcelona")) // Sorted by nombre
                .andExpect(jsonPath("$[1].nombre").value("Madrid"));

        verify(cityService).findCities();
        verify(mapper).map(city1, CityDto.class);
        verify(mapper).map(city2, CityDto.class);
    }
